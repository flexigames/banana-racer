...
import VehicleModel, { VehicleType } from './VehicleModel';
...
const RemotePlayer: React.FC<RemotePlayerProps> = ({ 
  playerId, 
  position, 
  rotation, 
  speed = 0, 
  ...
  useEffect(() => {
    ...
    const direction = new THREE.Vector3()
      .subVectors(positionVec, lastPosition.current)
      .normalize();
    ...
    targetPosition.current.copy(positionVec);
  ...
  // Get initial position as an array for the group
  const initialPosition: [number, number, number] = Array.isArray(position) 
    ? position 
    : [position.x, position.y, position.z];
  
  return (
    <group ref={car} position={initialPosition} rotation={[0, rotation, 0]}>
      <VehicleModel 
